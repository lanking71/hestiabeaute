...
interface InquiryListProps {
  inquiries: Inquiry[];
  total: number;   // 전체 게시물 수 (순번 계산용)
  page: number;    // 현재 페이지
  size: number;    // 페이지당 개수
}

export default function InquiryList({ inquiries, total, page, size }: InquiryListProps) {
  if (!inquiries.length) {
    return (
    ...
    <table className="w-full text-sm">
      ...
      <tbody>
        {inquiries.map((inquiry, index) => {
          // 최신글이 가장 큰 번호: total - (이전 페이지 게시물 수) - 현재 인덱스
          const rowNumber = total - (page - 1) * size - index;
          ...
          return (
            ...
            >
              <td className="py-3 px-4 text-hestia-gray">{rowNumber}</td>
